...
Future<UserCredential?> _signInWithGoogleAllPlatforms() async {
  ...
  // On Windows, use mock authentication to bypass Google sign-in
  if (PlatformService.isWindows) {
    debugPrint('Windows detected - using mock authentication');
    
    // Create mock credential and sign in
    final credential = GoogleAuthProvider.credential(
    ...
    // Set up mock user data in SharedPreferences
    SharedPreferencesUtil().email = 'test@example.com';
    SharedPreferencesUtil().givenName = 'Test';
    SharedPreferencesUtil().familyName = 'User';
    
    debugPrint('Mock sign-in completed - User: Test User (test@example.com)');
    return result;
  }
